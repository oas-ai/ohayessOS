...
공통 컴포넌트는 `Panel` `Caption` `Divider` `Readout` `StatusBadge` `EmptyState` `Icon`과 컨트롤 `OasButton` `OasIconButton` `OasToggle` `OasSlider` `OasSegmented` `OasStepper`, 셸 `StatusRail` `ControlDock` `SidePanel` `OasModal` `Toast` `NotificationItem` `CriticalOverlay` `CategoryTile`, 도메인 `VehicleVisual` `VehicleStatusIndicator` `TempZone` `MediaMiniPlayer` `NavInstruction` `MapSurface`다. 12개 화면은 `qml/Screen*.qml`이며 조합만 담당한다.
...
CTest가 drive·park·waiting·stale 상태와 12개 화면 전부, 라이트 테마 3종, 1280×720 / 1920×1080 / 2560×1440 / 3840×1200 네 해상도, R 기어 자동 전면화를 렌더링한다. QML 바인딩 오류·타입 로드 실패·바인딩 루프는 테스트 실패로 처리한다. 스크린샷은 CI 아티팩트이며, 테스트 상태뿐 아니라 이미지도 확인한다.

렌더러는 blur shader 없이 surface 단계와 1px border로 레이어를 표현하므로 software renderer와 임베디드 GPU가 같은 레이아웃을 만든다. 반복 애니메이션은 충전 표시와 방향지시등 두 곳뿐이다.
