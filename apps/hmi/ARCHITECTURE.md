...
The Gateway supervisor fans out snapshots to that FIFO and `ohayess-hmi.service` owns its lifecycle. `crates/viewer` deliberately uses a loopback HTTP endpoint only for development and browser CI; it is not a dependency of the production HMI.
