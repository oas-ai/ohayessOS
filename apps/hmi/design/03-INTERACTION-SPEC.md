...
| 규칙 | 근거 |
| --- | --- |
| 주요 기능은 2 touch 이내 | 런처 1 touch + 목적지 1 touch. 화면 안의 조작은 추가 1 touch. 3단계 depth를 만들지 않는다. |
...
| 런처 외부 탭 | 메뉴 닫기 | 그 외 scrim 동작 없음 |
| Pinch | 지도 확대만 | 그 외 전부 |

화면 전환은 런처 메뉴의 명시적 tap과 숫자키 1–8, 그리고 R 기어 자동 전면화뿐이다. swipe 전환을 제공하지 않는다.
...
| --- | --- | --- | --- |
| 화면 전환 | 260 | OutCubic | opacity 0→1 |
| 런처 메뉴 | 180 | OutCubic | opacity + y 상승 |
| 런처 버튼 반전 | 120 | OutCubic | color |
| 버튼 press | 120 | OutCubic | color |
| Toggle knob | 120 | OutCubic | x, 트랙 color |
| Segmented 선택 | 120 | OutCubic | color |
| Slider 값(외부 변경) | 180 | OutCubic | 채움 width |
| Slider 값(드래그 중) | 0 | — | 손가락 추종. 애니메이션 금지. |
| Metric 값 변경 | 180 | OutCubic | 값만 교체. flash 없음. |
| MeterBar / BarSeries | 180 | OutCubic | width / height |
| SidePanel | 260 | InOutCubic | x |
| Modal | 180 | OutCubic | opacity |
| Toast | 180 | OutCubic | opacity + y |
| Critical 출현 | **0** | — | 지연 없음 |
| Critical 소멸 | 180 | OutCubic | opacity |
| 도어 열림/닫힘 | 260 | InOutCubic | 패널 회전 + 채움 색 |
| 조향각 반영 | 180 | OutCubic | 바퀴 rotation |
...
- blur shader와 그림자를 쓰지 않는다. 레이어는 `line`과 surface 대비로만 표현한다.
- **매 프레임 움직이는 요소는 Canvas 밖의 QML 아이템으로 만든다.** Canvas를 타이머로 재그리면 software renderer의 이벤트 루프가 굶어 캡처·입력이 멈춘다.
- **레이아웃 안의 자식에 `anchors`를 걸지 않는다.** 셀 전체를 덮는 요소는 `Cell.overlay` 슬롯을 쓴다.
...
| 동작 | 피드백 | 시점 |
| --- | --- | --- |
| 온도 변경 | 수치 즉시 갱신 + 존 배경 `surfaceAlt` 전환 120ms | 드래그 중 |
...
| 런처의 Software / Diagnostics | 잠금 (메뉴에 사유와 함께 남음) |
...
| 안내 지점 200m 이내 | Home의 다음 안내 셀을 갱신. 화면 전환 없음. |
...
- `activeFocus`는 `ink` 2px 외곽선으로 항상 보인다. `focus: false` 스타일을 만들지 않는다.
- 키보드 Tab 순서는 시각 순서와 일치한다 (TopBar → 그리드 셀을 좌상단부터 행 우선으로 → 런처).
- 색만으로 상태를 구분하지 않는다. 형태 또는 텍스트를 동반한다.
- 숫자키 1–8로 앞의 여덟 목적지에 직접 이동한다. `Esc`는 런처 메뉴를 닫는다.
- 잉크 채움 위의 텍스트는 항상 `onInk`를 쓴다. 채움과 텍스트가 같은 계열로 겹치지 않게 한다.
