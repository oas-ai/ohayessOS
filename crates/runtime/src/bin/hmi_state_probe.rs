...
fn main() -> Result<(), String> {
    ...
    let mut length = [0; 4];
    input.read_exact(&mut length).map_err(|error| error.to_string())?;
    ...
    let diagnostics = HmiCapability::try_from(state.diagnostics).unwrap_or(HmiCapability::Unspecified);
    let controls = HmiCapability::try_from(state.vehicle_controls).unwrap_or(HmiCapability::Unspecified);
    println!(
        "freshness={freshness:?} media={media:?} reason={} diagnostics={diagnostics:?} controls={controls:?} speed={:?}",
        state.media_playback_reason,
        state.vehicle_state.and_then(|vehicle| vehicle.vehicle_speed_mps)
    );
    Ok(())
}
